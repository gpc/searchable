...
class DefaultSearchableCompassQueryBuilderTests extends AbstractSearchableCompassTests {
    ...
    void testBuildQueryWithString() {
        withCompassSession { compassSession ->
            ...
            shouldFail(ParseException) {
                query = builder.buildQuery(queryBuilder, "[special characters sometimes need to be escaped to avoid a runtime parse exception]", [class: Post, escape: false])
            }

            // Without escape, with class
            def mockQuery = new Mock(CompassQuery.class)
            def queryProxy = mockQuery.proxy()
            mockQuery.expects(new InvokeOnceMatcher()).method('setAliases').'with'(new IsEqual(["Post"] as String[])).will(new ReturnStub(queryProxy))
            ...
            builder.buildQuery(mockCqb.proxy(), "some typical search term", [escape: false, class: Post])
            ...
            mockQuery.expects(new InvokeOnceMatcher()).method('setAliases').'with'(new IsEqual(["Comment"] as String[])).will(new ReturnStub(queryProxy))
            ...
            builder.buildQuery(mockCqb.proxy(), "[special characters sometimes need to be escaped to avoid a runtime parse exception]", [class: Comment, escape: true])
    ...
    void testBuildQueryWithClosure() {
        ...
        withCompassSession { compassSession ->
            ...
            def queryBuilder = compassSession.queryBuilder()
            def query = builder.buildQuery(queryBuilder, [:]) {
            ...
            shouldFail {
                query = builder.buildQuery(queryBuilder, [escape: true]) {
...
//            mockCqb.expects(new InvokeOnceMatcher()).method('toQuery')//.'with'(new IsEqual("keywords"), new IsEqual('groovy')).will(new ReturnStub(cqbProxy))
            ...
            query = builder.buildQuery(cqbProxy, [class: Post]) {
